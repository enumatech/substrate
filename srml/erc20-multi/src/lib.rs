...
use rstd::prelude::*;
use support::{codec::{Decode, Encode}, dispatch::Result, Parameter, StorageMap, StorageValue, decl_storage, decl_module, decl_event, ensure};
...
decl_module! {
  pub struct Module<T: Trait> for enum Call where origin: T::Origin {
      ...
      pub fn init(origin, name: Vec<u8>, ticker: Vec<u8>, total_supply: T::TokenBalance) -> Result {
          ...
          Self::deposit_event(RawEvent::Transfer(token_id, sender.clone(), sender, total_supply));
      ...
      pub fn transfer(_origin, token_id: u32, to: T::AccountId, value: T::TokenBalance) -> Result {
      ...
      pub fn approve(_origin, token_id: u32, spender: T::AccountId, value: T::TokenBalance) -> Result {
          let sender = ensure_signed(_origin)?;
      ...
      pub fn transfer_from(origin, token_id: u32, from: T::AccountId, to: T::AccountId, value: T::TokenBalance) -> Result {
        ...
        ensure!(<Allowance<T>>::exists((token_id, from.clone(), spender.clone())), "Allowance does not exist.");
        ...
        Self::deposit_event(RawEvent::Approval(token_id, from.clone(), spender.clone(), value));
